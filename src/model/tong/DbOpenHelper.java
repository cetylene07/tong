...
//		Toast.makeText(mCtx, c.getCount()+"", Toast.LENGTH_SHORT).show();
	...
	public Cursor getAllIncalls()	{
		String query = "SELECT NAME, NUMBER, SUM(DURATION) FROM " + DataBases.CreateDB._TABLENAME + " WHERE TYPE=1 GROUP BY NAME HAVING SUM(DURATION)>0 ORDER BY SUM(DURATION) DESC";
		Log.d("query", "getAllIncomes has completed!");
		return mDB.rawQuery(query, null);
	}
	
	public Cursor getAllOutcalls()	{
		String query = "SELECT NAME, NUMBER, SUM(DURATION) FROM " + DataBases.CreateDB._TABLENAME + " WHERE TYPE=2 GROUP BY NAME HAVING SUM(DURATION)>0 ORDER BY SUM(DURATION) DESC";
